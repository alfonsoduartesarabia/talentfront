...
class LoginController(
    ...
    fun register(@RequestBody registerRequest: RegisterRequest): ResponseEntity<*> {
        ...
        return ResponseEntity.ok()
            .body(RegisterResponse(userId = savedUser.id!!, sessionCookie = sessionService.getUserSessionCookie(savedUser).toString()))
    }
    ...
    fun login(@RequestBody loginRequest: LoginRequest): ResponseEntity<*> {
        ...
        return if (verifyPassword(loginRequest.password, user)) {
            return ResponseEntity.ok()
                .body(LoginResponse(user.id!!, sessionService.getUserSessionCookie(user).toString()))
        } else {
